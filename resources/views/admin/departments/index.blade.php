    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.departments.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded text-sm">{{ __('Create Department') }}</a>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">{{ __('Manage departments for user and request assignment.') }}</p>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($departments as $department)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap space-x-2">
                                        <a href="{{ route('admin.departments.edit', $department) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('Edit') }}</a>
                                        <form action="{{ route('admin.departments.destroy', $department) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('Are you sure?') }}');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="2" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">{{ __('No departments found.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
