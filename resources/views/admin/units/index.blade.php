    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.units.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded text-sm">{{ __('Create Unit') }}</a>
            </div>
        </div>
    </x-slot>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead>
                            <tr>
                                ...
                                <th class="px-6 py-3 bg-gray-50 dark:bg-gray-700 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">{{ __('Symbol') }}</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($units as $unit)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap space-x-2">
                                        <a href="{{ route('admin.units.edit', $unit) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('Edit') }}</a>
                                        <form action="{{ route('admin.units.destroy', $unit) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('Are you sure?') }}');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">{{ __('No units found.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
